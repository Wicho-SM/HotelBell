...
namespace HotelBell
{
    class ConexionRecepcionista
    {
        ...
        public static void checkIn(ComboBox comboBoxClientes, TextBox textBTelefono, TextBox textBEmail,
            TextBox textBHabitacion, int validarDatos, TextBox fechaReservacion)
        {
            bool abrirConexion = false;
            MySqlConnection conectar = new MySqlConnection("server = 127.0.0.1; database = hotel_reservaciones; Uid = root; pwd =");
            try
            ...
            else if (validarDatos == 3)
            {
                string queryBox = "SELECT fechaIngreso FROM reservaciones WHERE nombre = '" + comboBoxClientes.Text + "'";
                ...
                if (abrirConexion == true)
                {
                    ...
                    while (almacena.Read())
                    {
                        fechaReservacion.Text = almacena.GetValue(0).ToString();
                    }
            ...
            TextBox textBHabitacion, TextBox textBEntrada, TextBox textBSalida, int validarDatos)
        {
            ...
            {
                string queryBox = "UPDATE habitacion SET ocupada = '0' WHERE habitacion.habitacion = '" + textBHabitacion.Text + "'";
